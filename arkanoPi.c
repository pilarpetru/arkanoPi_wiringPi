...
TipoLedDisplay led_display = {
	...
	.filas = {
		// A completar por el alumno...
		GPIO_LED_DISPLAY_ROW_1, GPIO_LED_DISPLAY_ROW_2, GPIO_LED_DISPLAY_ROW_3,
		...
		if(kbhit()) {
			...
			switch(teclaPulsada) {
				...
				case 'a':
					...
					printf("Movimiento a la izquierda!\n");
					fflush(stdout);
				break;
				...
				case 'd':
					...
					piUnlock (SYSTEM_FLAGS_KEY);
					printf("Movimiento a la derecha!\n");
					fflush(stdout);
				break;
...
int main () {
	...
	//tmr_t* tmr_refresco_display = tmr_new (timer_refresco_display_isr);
	...
	// tmr_destroy ((tmr_t*)(display_fsm->user_data));
